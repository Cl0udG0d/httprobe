...
func main() {
	...
	var statusCodesRaw string
	flag.StringVar(&statusCodesRaw, "status-code", "", "set the expected HTTP status codes (comma-separated)")
	...
	var statusCodes []int
	if statusCodesRaw != "" {
		codeStrings := strings.Split(statusCodesRaw, ",")
...
func isListening(client *http.Client, url, method string, codes []int) bool {
	...
	if resp != nil && isStatusCodeInSlice(resp.StatusCode, codes) {
		return true
	}
...
// isStatusCodeInSlice 检查给定的状态码是否在切片中
func isStatusCodeInSlice(statusCode int, statusCodes []int) bool {
	if statusCodes == nil {
		statusCodes = []int{200, 302, 403}
	}
